...
#define PRINTXD(FUNCTION_NAME, NTSTATUS_ERROR) \
    fprintf(stderr, "%s %d: %s error = 0x%lx\n", __FILE__, __LINE__, FUNCTION_NAME, NTSTATUS_ERROR)
...
static inline UINT_PTR GetNtFunctionAddress(LPCSTR FunctionName, HMODULE ModuleHandle) {
...
BOOL DirectSyscallsInjection(_In_ CONST DWORD PID, _In_ CONST SIZE_T PayloadSize);
...
static inline void InitAnsiString(ANSI_STRING* str, const char* funcName) {
